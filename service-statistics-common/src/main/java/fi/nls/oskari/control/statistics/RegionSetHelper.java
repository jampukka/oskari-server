...
public class RegionSetHelper {
    ...
    protected static SimpleFeatureCollection getRegionsResources(RegionSet regionset, String requestedSRS)
        ...
        if (path.endsWith(".zip") || path.endsWith(".shp")) {
            return getRegionsResourcesShape(regionset, requestedSRS, path);
        }
        throw new IllegalArgumentException("Invalid resource file format");
    }

    /**
     * Read (Simple)FeatureCollection from ESRI shape file
     * transforming geometries to the requestedSRS
     */
    private static SimpleFeatureCollection getRegionsResourcesShape(RegionSet regionset, String requestedSRS,
            String path) {
    ...
    protected static JSONObject getGeoJSON(SimpleFeature feature, String idProp, String nameProp) throws IOException {

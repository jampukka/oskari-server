...
public class HeightmapProcessor implements Runnable {
    ...
    public HeightmapProcessor(String endPoint, Capabilities caps, RectifiedGridCoverage desc, double[] allowedExtent, int maxZoom, File dir, int z, int c, int r) {
    ...
    public void run() {
        ...
        if (mask != -1) {
            ...
            if ((mask & MASK_SOUTH_WEST) != 0) {
                new HeightmapProcessor(endPoint, caps, desc, allowedExtent, maxZoom, dir, z, c, r).run();
            }
            if ((mask & MASK_SOUTH_EAST) != 0) {
                new HeightmapProcessor(endPoint, caps, desc, allowedExtent, maxZoom, dir, z, c + 1, r).run();
            }
            if ((mask & MASK_NORTH_WEST) != 0) {
                new HeightmapProcessor(endPoint, caps, desc, allowedExtent, maxZoom, dir, z, c, r + 1).run();
            }
            if ((mask & MASK_NORTH_EAST) != 0) {
                new HeightmapProcessor(endPoint, caps, desc, allowedExtent, maxZoom, dir, z, c + 1, r + 1).run();
            }
            return;
        }
        ...
        try {
            FloatGeoTIFF image = foo(apprExtentTM35);
    ...
    private void storeTerrain(int zoom, int col, int row, FloatGeoTIFF image, HeightPoint[] points, byte[] hmap) {
        ...
        int mask = bar(image, points, hmap, zoom == maxZoom);
        try {
            System.out.printf("%d/%d/%d, %s%n",
                    zoom, col, row, Arrays.toString(bounds));
    ...
    private int checkApproximateExtent(double[] extentTM35, RectifiedGridCoverage desc, byte[] hmap) {
        ...
        double sizeM = Math.max(eMax - eMin, nMax - nMin);
        if (sizeM > 10000) {
            System.arraycopy(EMPTY, 0, hmap, 0, EMPTY.length);
    ...
    private FloatGeoTIFF foo(double[] extentTM35) throws ServiceException {
    ...
    private int bar(FloatGeoTIFF image, HeightPoint[] points, byte[] hmap, boolean maxZoom) {
        int off = 0;
        int mask = 0;
    ...
    public static FloatGeoTIFF getImage(String endPoint, GetCoverage getCoverage) throws ServiceException {
        ...
        try {
            ...
            if (200 != conn.getResponseCode()) {
                try (InputStream in = conn.getErrorStream()) {
                    byte[] err = IOHelper.readBytes(conn.getErrorStream());
                    throw new ServiceException(new String(err, StandardCharsets.UTF_8));
                }
            } else {
                try (InputStream in = conn.getInputStream()) {
                    response = IOHelper.readBytes(in);
                }
            }
        } catch (IOException e) {
        ...
        try {
            return new FloatGeoTIFF(response);
